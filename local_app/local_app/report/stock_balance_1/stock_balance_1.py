...
class StockBalance1Filter(TypedDict):
	company: str | None
	from_date: str
	to_date: str
	item_group: str | None
...
def execute(filters: StockBalance1Filter | None = None):
	return StockBalance1Report(filters).run()


class StockBalance1Report:
	def __init__(self, filters: StockBalance1Filter | None) -> None:
	...
	def run(self):
		...
		data1 = self.data
		for row in data1:
			row['stock_after_reservation'] = row['bal_qty'] - row['reserved_stock']
		
		return self.columns, data1
	...
	def prepare_new_data(self):
		...
		for _key, report_data in self.item_warehouse_map.items():
			...
			# frappe.error_log("+++++++++++++++++++++++++++++++++")
			# report_data.update(
			# 	{"stock_after_reservation": sre_details.get((report_data.item_code, report_data.warehouse)-report_data.bal_qty,0.0)}
			# )
			if (
	...
	def get_columns(self):
		...
		columns.extend(
			[
				...
				{
					"label": _("Stock after Reservation"),
					"fieldname": "stock_after_reservation",
					"fieldtype": "Float",
					"width": 80,
					"convertible": "qty",
				},
